...
<form class="form-horizontal" role="form" name="frm">	
	...
	<div class="portlet light bordered">
		...
		<div class="row">                     
                <div class="col-md-6">
                    <div><b>Thời gian thử việc</b>  </div>
...
<script type="text/javascript" language="javascript">	
	...
	function getList() {  	
		...
		$.ajax({
			...
			success: function (data, status, jqXHR) {  								
				vProbationaryTable.clear().draw();
				vProbationaryTable.rows.add(data).draw();
				spinner.hide();
			},
	...
	function checkWithList(this_checkbox){
		var dr = vProbationaryTable.row( $(this_checkbox).closest('tr') ).data();       		
		...
		vProbationaryTable.row( $(this_checkbox).closest('tr') ).data(dr);
	}	
	function changeStatus(id,status){		
		...
		$.ajax({
			...
			success: function (data, status, jqXHR) {   							                              				
				showMsg('note',data);               		
				vProbationaryTable.clear().draw();
				vProbationaryTable.rows.add(data.data).draw();
				spinner.hide();
			},
			beforeSend  : function(jqXHR,setting){
				spinner.show();
	...
	function updateStatus(status){				
		var token 	= 	$('input[name="_token"]').val();   		
		var dt 		= 	vProbationaryTable.data();		
		var str_id	=	"";		
