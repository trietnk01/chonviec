...
<form class="form-horizontal" role="form" name="frm">	
	...
	<div class="portlet light bordered">
		...
		<div class="row">                     
                <div class="col-md-6">
                    <div><b>Hình thức làm việc</b>  </div>
...
<script type="text/javascript" language="javascript">	
	...
	function getList() {  	
		...
		$.ajax({
			...
			success: function (data, status, jqXHR) {  								
				vWorkingFormTable.clear().draw();
				vWorkingFormTable.rows.add(data).draw();
				spinner.hide();
			},
	...
	function checkWithList(this_checkbox){
		var dr = vWorkingFormTable.row( $(this_checkbox).closest('tr') ).data();       		
		...
		vWorkingFormTable.row( $(this_checkbox).closest('tr') ).data(dr);
	}	
	function changeStatus(id,status){		
		...
		$.ajax({
			...
			success: function (data, status, jqXHR) {   							                              				
				showMsg('note',data);               		
				vWorkingFormTable.clear().draw();
				vWorkingFormTable.rows.add(data.data).draw();
				spinner.hide();
			},
			beforeSend  : function(jqXHR,setting){
				spinner.show();
