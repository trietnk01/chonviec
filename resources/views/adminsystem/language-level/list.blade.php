...
<form class="form-horizontal" role="form" name="frm">	
	...
	<div class="portlet light bordered">
		...
		<div class="row">                     
                <div class="col-md-6">
                    <div><b>Trình độ ngoại ngữ</b>  </div>
...
<script type="text/javascript" language="javascript">	
	...
	function getList() {  	
		...
		$.ajax({
			...
			success: function (data, status, jqXHR) {  								
				vLanguageLevelTable.clear().draw();
				vLanguageLevelTable.rows.add(data).draw();
				spinner.hide();
			},
	...
	function checkWithList(this_checkbox){
		var dr = vLanguageLevelTable.row( $(this_checkbox).closest('tr') ).data();       		
		...
		vLanguageLevelTable.row( $(this_checkbox).closest('tr') ).data(dr);
	}	
	function changeStatus(id,status){		
		...
		$.ajax({
			...
			success: function (data, status, jqXHR) {   							                              				
				showMsg('note',data);               		
				vLanguageLevelTable.clear().draw();
				vLanguageLevelTable.rows.add(data.data).draw();
				spinner.hide();
			},
			beforeSend  : function(jqXHR,setting){
				spinner.show();
	...
	function updateStatus(status){				
		var token 	= 	$('input[name="_token"]').val();   		
		var dt 		= 	vLanguageLevelTable.data();		
		var str_id	=	"";		
